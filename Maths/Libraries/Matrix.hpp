...
friend Matrix matrixFromVector(const Matrix& vec);
friend Matrix vectorFromMatrix(const Matrix& mat);
friend Quaternion quaternionFromVector(const Matrix& vec);
...
public:
    ...
    Matrix at(unsigned int ind) const;
    ...
    // returns Identinty matrix of size nxn
    static Matrix I(const unsigned int& n);
    ...
    public:
        virtual const char *what() const noexcept
        {
            return "The concerned matrix is not a 1x3 vector in R^3!";
        }
    };
    class Not3By1VectorEXCEPTION : public std::exception
    {
    public:
        virtual const char *what() const noexcept
        {
            return "The concerned matrix is not a 3x1 vector in R^3!";
        }
    };
    ...
    class NotRotationMatrixEXCEPTION : public std::exception
    {
    public:
        virtual const char *what() const noexcept
        {
            return "Concerned Matrix is not a Rotation Matrix in R^3!";
        }
    };
};
...
std::ostream &operator<<(std::ostream &os, const Matrix &matrix)
{
    ...
    matrix.print(os);
    os.unsetf(std::ios::scientific | std::ios::fixed);
    return os;
}
